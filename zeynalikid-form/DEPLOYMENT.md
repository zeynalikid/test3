...
Deploy `zeynalikid-form` as a Vite application using the included `vercel.json`.
...
Never commit a real `.env` file, password, token, API key, or other secret. Supabase and application URLs must be configured only through the deployment environment.
